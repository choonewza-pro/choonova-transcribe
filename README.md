...
| `WHISPERX_MODEL`             | `large-v3-turbo`          | No       | WhisperX ASR model for English/Auto + diarization (Path 4) and the plain `whisperx` selection. `whisperx.load_model()` passes the name straight to faster-whisper, so a faster-whisper CT2 repo id or a plain Whisper size name both work. The `whisperx-thai` selection overrides this with the Thai-tuned CT2 model (`WHISPER_THAI_MODEL`) for accurate Thai text. |
